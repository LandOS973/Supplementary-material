...
def _resolve_data_file(script_dir: str, data_file: str | None) -> str:
    if data_file:
...
def main():
    ...
    data_file = _resolve_data_file(script_dir, args.data_file)
    from bbdob import NasBench101
    objective = NasBench101(filename=data_file)

    factory = FactoryStrategyEA()

    dim = 26
    dim_variables = [2 for _ in range(21)] + [3 for _ in range(5)]
