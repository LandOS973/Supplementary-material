...
def _oracle_sanity_check(task, dim: int, alphabet_size: int, verbose: bool = True) -> None:
    if not verbose:
        return
    ...
    # Compare with task.x/task.y if available to validate oracle behavior.
    try:
...
def main(cfg: DictConfig):
    ...
    if problem_name in ("designbench", "gfp", "gfp-v0"):
        task_name = str(OmegaConf.select(cfg, "problem.task_name") or "GFP-v0")
        dim = int(OmegaConf.select(cfg, "problem.dim") or 237)
        alphabet_size = int(OmegaConf.select(cfg, "problem.alphabet_size") or 20)
        oracle_batch_size = int(OmegaConf.select(cfg, "problem.oracle_batch_size") or cfg.get("oracle_batch_size") or 2048)
    else:
        print("[WARN] problem config is not designbench; using defaults task=GFP-v0 dim=237 alphabet=20")
        task_name = "GFP-v0"
        dim = 237
        alphabet_size = 20
        oracle_batch_size = int(cfg.get("oracle_batch_size", 2048))
    ...
    # Force SVGD prints at every iteration for this runner (no YAML changes needed).
    ...
    task = design_bench.make(task_name, use_cuda=torch.cuda.is_available())

    def _infer_task_space(task_obj, dim_val: int, alpha_val: int):
    ...
    inferred_dim, inferred_alpha, inferred_mode, discrete_ok = _infer_task_space(task, dim, alphabet_size)
    ...
    _oracle_sanity_check(task, dim=dim, alphabet_size=alphabet_size, verbose=verbose)
