...
def render_agent_dashboard(
    ...
    try:
        ...
        def draw_fitness_axis(total_rows, row):
        ...
        def draw_metrics():
            nonlocal fitness_ax, fitness_lines, hamming_evo_ax
            fig.clear()
            plot_handles.clear()
            overlay_lines.clear()
            ...
            total_rows = (
                total_metric_rows
                + (1 if is_fitness_enabled() else 0)
            ...
            if fitness_ax is not None:
                row += 1
            hamming_evo_ax = draw_hamming_evo_axis(total_rows, row)
            if hamming_evo_ax is not None:
                row += 1
            last_axis = hamming_evo_ax if hamming_evo_ax is not None else (
                fitness_ax if fitness_ax is not None else (
