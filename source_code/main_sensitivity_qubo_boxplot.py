#!/usr/bin/env python3
"""
Plot sensitivity boxplots for QUBO from existing config results.
...
3) For each QUBO instance folder (QUBO_dimN_tT), reads the LAST row of best_metrics.csv.
4) Builds boxplots using the stored quantiles (2%, 25%, 50%, 75%, 98%) and mean.
5) Writes a summary CSV + saves plots.

QUBO scores are displayed as positive values (absolute).
"""
...
def _abs_transform(stats: Dict[str, float]) -> Dict[str, float]:
    out = {}
    for key, value in stats.items():
        if value is None:
            ...
            out[key] = value
    # Ensure ordering for boxplot stats after abs transform
...
def load_qubo_stats_for_config(config_dir: Path) -> Dict[Tuple[int, int], Dict[str, float]]:
    """Return {(n, type_instance): stats} for a single config directory."""
    stats_by_instance: Dict[Tuple[int, int], Dict[str, float]] = {}
    for entry in config_dir.iterdir():
        if not entry.is_dir():
            continue
        match = re.match(r"^QUBO_dim(?P<n>\d+)_t(?P<t>\d+)$", entry.name)
        if not match:
            continue
        n = int(match.group("n"))
        t = int(match.group("t"))
        ...
        if not stats:
            continue
        stats_by_instance[(n, t)] = _abs_transform(stats)
...
def plot_boxplots(stats_map: Dict[Tuple[int, int], Dict[int, Dict[str, float]]], output_dir: Path) -> None:
    ...
    for (n, t), per_m in sorted(stats_map.items(), key=lambda item: (item[0][0], item[0][1])):
        ...
        ax.set_title(f"QUBO (N={n}, Type={t})", fontsize=18, fontweight="bold", pad=20)
        ...
        plot_path = output_dir / f"boxplot_QUBO_{n}_type{t}.png"
...
def main() -> None:
    ...
    # Build stats per (n, type) and per M
    stats_map: Dict[Tuple[int, int], Dict[int, Dict[str, float]]] = {}
    summary_rows: List[List[str]] = []

    for m, cfg_dir in variants:
        per_instance = load_qubo_stats_for_config(cfg_dir)
        for (n, t), stats in per_instance.items():
            stats_map.setdefault((n, t), {})[m] = stats

            # summary row (abs values)
            summary_rows.append([
                "QUBO",
                str(n),
                str(t),
    ...
    summary_csv = output_dir / "sensitivity_qubo_from_config.csv"
