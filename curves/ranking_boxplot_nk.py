...
COMPETITORS = [
    "DiscreteDE",
    "RecombiningDiscreteLenglerOnePlusOne",
    "SmoothRecombiningDiscreteLenglerOnePlusOne",
]

DISPLAY_NAMES = {
    "RecombiningDiscreteLenglerOnePlusOne": "RDL1+1",
    "SmoothRecombiningDiscreteLenglerOnePlusOne": "S-RDL1+1",
}
...
# Whiskers/caps beyond this value are truncated for readability, since a
# single algorithm (DiscreteDE) has a long tail that would otherwise squash
# the rest of the plot; the true value is annotated at the cut point.
AXIS_CUTOFF = 26
...
def plot(ranks: dict[str, list[int]], output_path: Path) -> None:
    ...
    for i, d in enumerate(data):
        whisker = bp["whiskers"][2 * i + 1]
        cap = bp["caps"][2 * i + 1]
        whisker_end = cap.get_xdata()[0]
        if whisker_end <= AXIS_CUTOFF:
            continue
        whisker.set_xdata([whisker.get_xdata()[0], AXIS_CUTOFF])
        cap.set_xdata([AXIS_CUTOFF, AXIS_CUTOFF])
        ax.annotate(
            f"⇥ {whisker_end:.0f}",
            xy=(AXIS_CUTOFF, i),
            xytext=(4, 0),
